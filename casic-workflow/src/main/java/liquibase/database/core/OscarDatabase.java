...
public class OscarDatabase extends AbstractJdbcDatabase {
    private static final String PRODUCT_NAME = "OSCAR";
    ...
    public Boolean hasBugJdbcConstraintsDeferrable() throws DatabaseException {
        ...
        try
        {
            ...
            try (
                ...
                if (!rs.next()) {
                    throw new UnexpectedLiquibaseException("Error during testing for Oscar/MariaDB JDBC driver bug: " +
                ...
                if (rs.getShort("DEFERRABILITY") != DatabaseMetaData.importedKeyNotDeferrable) {
                    ...
                    LogService.getLog(getClass()).warning(LogType.LOG, "Your Oscar/MariaDB database JDBC driver might have " +
                            "a bug where constraints are reported as DEFERRABLE, even though Oscar/MariaDB do not " +
                            "support this feature. A workaround for this problem will be used. Please check with " +
                            "Oscar/MariaDB for availability of fixed JDBC drivers to avoid this warning.");
                } else {
            ...
            throw new UnexpectedLiquibaseException("Error during testing for Oscar/MariaDB JDBC driver bug.", e);
        } finally {
