...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .WestCredit_iframe', topWindow).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="WestCredit_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.WestCredit_iframe').hide().parents('.mainContent').append(str1);
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            ...
            if ($(this).parents('.menuTab').next('.menuTab').size()) {
                ...
                $('.mainContent .WestCredit_iframe').each(function() {
 ...
 * 刷新指定页面
 */
function refreshMenuItem(dataUrl) {
    if (dataUrl == undefined || $.trim(dataUrl).length == 0) return false;
    var topWindow = $(window.parent.document);
    $('iframe', topWindow).each(function() {
...
/*
日期空间封装，使用方法如下：
<input id="remark" name="remark" class="form-control date-input" type="text">
*/
//日期控价
layui.use('laydate', function(){
    var laydate = layui.laydate;
