/**
 * 通用js方法封装处理
 * Copyright (c) 2018 WestCredit_iframe
 */
(function ($) {
    $.extend({
    	_treeTable: {},
    	...
    	table: {
            ...
            init: function(options) {
                ...
                //console.info("options : "+options.url);
                //console.info("is empty : "+$.common.isEmpty(options.queryParams));
                
                this._params = $.common.isEmpty(options.queryParams) ? this.queryParams : options.queryParams;
                ...
                var height = $.common.isEmpty(options.height) ? "" : options.height;
                tableId =  $.common.isEmpty(options.tableId)? '#bootstrap-table' : options.tableId;
              ...
              //      data:options.data,
                    ...
                    pageSize: $.common.isEmpty(options.pageList)?10:options.pageSize,                                       // 每页的记录行数（*）
                    pageList: $.common.isEmpty(options.pageList)?[10, 25, 50]:options.pageList,                             // 可供选择的每页的行数（*）
                    height: height,
                    ...
                    responseHandler: options.responseHandler? options.responseHandler:this.responseHandler,            // 回调函数
    		...
    		exportExcel: function(formId) {
    			...
    			$.post(this._option.exportUrl, $("#" + currentId).serializeArray(), function(result) {
            ...
            // 刷新
            refresh: function(selTableId,selUrl) {
                $( $.common.isEmpty(selTableId)?tableId:selTableId).bootstrapTable('refresh', {
                    url: $.common.isEmpty(selUrl)?this._option.url:selUrl,
                    silent: true
                });
            },
            // 查询选中列值
            ...
            selectFirstColumns: function() {
            	return $.map($(tableId).bootstrapTable('getSelections'), function (row) {
        	        return row[this._option.columns[1].field];
        	    });
            },
            ...
            // 回显数据
            selectSysNameLabel: function(datas, value) {
            	var actions = [];
                $.each(datas, function(index, syscfg) {
                    if (syscfg.sysCode == value) {
                    	actions.push("<span class='badge'>" + syscfg.sysName + "</span>");
                        return false;
                    }
                });
                return actions.join('');
            },
            importExcel1: function (formId) {
                ...
                layer.open({
                    ...
                    maxmin: true,
                    shade: 0.3,
                    title: '导入' + this._option.modalName + '数据',
                    content: $('#' + currentId),
                    ...
                    btn1: function (index, layero) {
                        ...
                        var param = $.common.isEmpty($("#param").val()) ? '' : $("#param").val();
                        formData.append("updateSupport", $("input[name='updateSupport']").is(':checked'));
                        $.ajax({
                            url: this._option.importUrl + "/" + param,
                            data: formData,
                            cache: false,
                            contentType: false,
                            processData: false,
...
//                                    $.modal.alertSuccess(result.msg);
                                    $(tableId).bootstrapTable('append', result.data);
                                    layer.closeAll();
                                }  else {
                                    layer.close(index);
                                    $.modal.alertError(result.msg);
                                    window.location.href = ctx + "common/download?fileName=" + result.fileName
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            muilParamsOpen: function (title, url, width, height,data) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层全屏
            topOpenFull: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            closeTab: function (dataId) {
                var targetTab = $(".menuTabs .menuTab[data-id='"+ dataId +"']",window.parent.document);
                var closeTabId = dataId
                var currentWidth = targetTab.width();

                // 当前元素处于活动状态
                if (targetTab.hasClass('active')) {

                    // 当前元素后面有同辈元素，使后面的一个元素处于活动状态
                    if (targetTab.next('.menuTab').size()) {

                        var activeId = targetTab.next('.menuTab:eq(0)').data('id');
                        targetTab.next('.menuTab:eq(0)').addClass('active');

                        $('.mainContent .WestCredit_iframe',window.parent.document).each(function() {
                            if ($(this).data('id') == activeId) {
                                $(this).show().siblings('.WestCredit_iframe').hide();
                                return false;
                            }
                        });

                        var marginLeftVal = parseInt($('.page-tabs-content',window.parent.document).css('margin-left'));
                        if (marginLeftVal < 0) {
                    ...
                    if (targetTab.prev('.menuTab').size()) {
                        var activeId = targetTab.prev('.menuTab:last').data('id');
                        targetTab.prev('.menuTab:last').addClass('active');
                        $('.mainContent .WestCredit_iframe',window.parent.document).each(function() {
                            if ($(this).data('id') == activeId) {
                                $(this).show().siblings('.WestCredit_iframe').hide();
                                return false;
        ...
        operate: {
            ...
            // 添加信息
            add: function(id, url, width, height) {
        	    if($.common.isEmpty(url)){
            ...
            // 修改信息
            edit: function(id, url, width, height) {
            	var url = "/404.html";
            ...
            // 添加信息
            addMore: function(id, url, title, width, height) {
                if($.common.isEmpty(url)){
                    console.log("添加" + title + "失败！url为空！");
                    return ;
                }
                url = $.common.isEmpty(id) ? url : url.replace("{id}", id);
                $.modal.open("添加" + title, url, width, height);
            },
            // 修改信息
            editMore: function(id, url, title, width, height) {
                if($.common.isEmpty(url)){
                    console.log("修改" + title + "失败！url为空！");
                    return ;
                }
                if ($.common.isNotEmpty(id)) {
                ...
                //     return;
                // }
                $.modal.open("修改" + title, url, width, height);
            },
            // 工具栏表格树修改
            editTree: function() {
...
//    		    }
		...
		workflow:{
			 ...
			 * params属性：
			 * 	container 按钮容器选择器
			 * 	btnData  流程接口输出的路由信息json
			 * 	candidateVar  流程图候选人配置的变量
			 * 	conditionVar  流程图网关条件配置的变量
			*/
    		_params:{},
    		buildButtons: function (params) {
                var container = $(params.container);
                var routeListJson = params.btnData;
                routeListJson = routeListJson.replace(/&quot;/g, '\"');
                var routeList = JSON.parse(routeListJson);
                var buttons = "";
                var backTarget = "";
                $.each(routeList,function (i,val) {
                    if(val.transType && val.transType=='BACK'){
                        backTarget = val.targetId;
                    }
                })
                for(var i=0; i<routeList.length; i++){
	                	var btnParams = {};
	                	btnParams.activeTaskId = params.activeTaskId;
	                	btnParams.processInstanceId = params.processInstanceId;
	                	btnParams.candidateGroups = routeList[i].candidateGroups;
	                	btnParams.candidateUsers = routeList[i].candidateUsers;
	                	btnParams.taskType = routeList[i].taskType;
	                	btnParams.candidateVar = params.candidateVar;
	                	btnParams.conditionVar = params.conditionVar;
	                	btnParams.condition = routeList[i].condition;
	                	btnParams.entityId = params.entityId;
	                	btnParams.transType = routeList[i].transType;
	                	btnParams.targetId = routeList[i].targetId;
	                	btnParams.backTarget = backTarget;
	                	var btnParamsJson = JSON.stringify(btnParams);
	                	if(!btnParams.transType || btnParams.transType != 'BACK'){
	                        buttons += '<a class="btn btn-success" condition="' + btnParams.condition + '" onclick=\'$.workflow.flowDialog(' + btnParamsJson + ')\' >' + routeList[i].name +'</a> ';
                    }

                }
                container.append(buttons);
            },
            flowDialog: function(btnParams) {
